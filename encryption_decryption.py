...
print("Press 2 for enter the file name ")
x = int(input())
if(x==1):
...
def store_decoded_str(s):
	fd = open("decode.txt", mode = "w")
	fd.write(s)
	fd.close()

def store_encoded_str(s):
	fd = open("encode.txt.ENC" , mode = "w")
	fd.write(s)
	fd.close()
...
while(1):
	...
	if(choice==1):
		...
		print("---------------ENCRYPTED FILE SAVED IN THE DIRECTORY-------------------")
		print()
		print("--- Time to encrypt %s seconds ---" % (time.time() - start_time))
		...
		print()
		print("--- Time to decrypt %s seconds ---" % (time.time() - start_time))
